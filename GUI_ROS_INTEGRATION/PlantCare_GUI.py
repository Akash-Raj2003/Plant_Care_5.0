...
from PyQt5.QtGui import QColor, QFont, QImage, QPainter, QPalette, QPen, QTextCursor
...
class CameraDialog(QDialog):
    def __init__(self, parent=None):
        ...
        image_label.setGeometry(50, 50, 500, 400)
        image_label.setAlignment(Qt.AlignCenter)
        image_label.setStyleSheet("background-color: white; border: 1px solid black;")

        if IMAGE_PATH.exists():
            image = QImage(str(IMAGE_PATH))
            if not image.isNull():
                image_label.setPixmap(
                    image.scaled(500, 400, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                )
            else:
                image_label.setText("Unable to load image")
        else:
            image_label.setText("flower.png not found")
