...
RIDGEBACK_INTERRUPTED_STATE = 5

class GreenhouseManager:
    def __init__(self, greenhouse_name):
        rospy.init_node('greenhouse_manager')
        
        self.filename = f"/home/sope/catkin_ws/src/my_simulations/config/{greenhouse_name}.json"
        ...
        self.ui_moisture_pub = rospy.Publisher('/plant/moisture_alert_sim', String, queue_size=10)
        ...
        rospy.Subscriber(GUI_COMMAND_TOPIC, String, self.gui_command_callback)
    ...
    def activate_emergency_stop(self):
        ...
        self.ui_state_pub.publish(RIDGEBACK_INTERRUPTED_STATE)
        self.ui_feedback_pub.publish("0,Emergency stop")
        self.ui_plan_pub.publish("")
    ...
    def resume_dispatch(self):
        ...
        self.emergency_stop_active = False
        self.ui_feedback_pub.publish("0,Resuming dispatch")

    def wait_while_stopped(self):
        while not rospy.is_shutdown() and self.emergency_stop_active:
            self.ui_state_pub.publish(RIDGEBACK_INTERRUPTED_STATE)
            rospy.sleep(0.1)

    def ensure_move_base_inactive(self, timeout=2.0):
        if self.emergency_stop_active:
            return False
        ...
        if state in active_states:
            rospy.loginfo("Cancelling active move_base goal before requesting /move_base/make_plan.")
            self.client.cancel_goal()
            deadline = rospy.Time.now() + rospy.Duration(timeout)

            while not rospy.is_shutdown() and self.client.get_state() in active_states:
                if rospy.Time.now() >= deadline:
                    rospy.logwarn("move_base did not become inactive before planning request.")
                    return False
                rospy.sleep(0.05)
    ...
    def get_path_distance(self, target_x, target_y):
        ...
        if not self.ensure_move_base_inactive():
            return float('inf')
    ...
    def run_mission(self):
        ...
        while not rospy.is_shutdown():
            ...
            while not rospy.is_shutdown():
                if self.emergency_stop_active:
                    rospy.logwarn("Emergency stop active. Cancelling navigation.")
                    self.client.cancel_goal()
                    self.publish_zero_velocity()
                    break
                ...
                if state == 3 or dist_to_goal < self.SUCCESS_DISTANCE_THRESHOLD:
                    ...
                    for i in range(0, 101, 25):
                        if self.emergency_stop_active:
                            rospy.logwarn("Emergency stop active. Cancelling watering sequence.")
                            self.publish_zero_velocity()
                            break
                ...
                if state in [4, 5, 9] or (rospy.get_time() - start_t > self.MAX_TOTAL_TIME):
                    self.client.cancel_goal()
                    ...
                    if current_fails == 1:
                        ...
                        self.plants[target]['last_fail_time'] = rospy.get_time()
